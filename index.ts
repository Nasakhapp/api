...
if (cluster.isPrimary) {
  ...
  app.get(
    ...
    async (
      ...
      if (updatedRequest.naji?.id)
        socketServer.emit(updatedRequest.naji?.id, {});
  ...
  app.post(
    ...
    async (
      req: express.Request<
        {},
        {},
        { amount: number; lat: number; long: number }
      >,
      res: express.Response
    ) => {
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      if (user?.walletAddress) {
        try {
          ...
          res
            .status(200)
            .json({ balance: fromNano(balance).toString(), jetton });
        } catch (error) {
          console.log(error);
        }
      }
  ...
  function telegramAuthMiddleware(
    req: express.Request,
    res: express.Response,
    next: express.NextFunction
  ) {
